 ...
 * 设计要点：
 *
 * 1. 单格 12px × 12px，53 列 × 7 行，本周固定在最右列
 ...
 * 用量热力图主组件。
 */
export function Heatmap({ cells, loading = false, metric }: HeatmapProps): ReactNode {
  ...
  const handleCellEnter = useCallback(
    (e: React.MouseEvent, cell: HeatmapCell) => {
      ...
      const text =
        value === 0
          ? `${cell.day} · 无请求`
          : `${cell.day} · ${formatNumber(cell.request_count)} 次请求 · ${formatNumber(cell.total_tokens)} 词元`;
