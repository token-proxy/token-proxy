...
export default function RequestConfigSection({
  ...
  const thinking = requestBody.thinking as { type?: string; budget_tokens?: number } | undefined;
  if (thinking?.type) {
  ...
  const outputConfig = requestBody.output_config as { effort?: string } | undefined;
  ...
  return (
    <AccordionSection title="请求配置" defaultExpanded={false}>
      <Descriptions row size="small" data={items.map((d) => ({ key: d.key, value: d.value }))} />
    </AccordionSection>
  );
}
