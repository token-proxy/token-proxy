...
export default function RequestContentCard({
  ...
  const formattedView = useMemo(() => {
    ...
    const system = requestBody.system as
    ...
    const hasAnySection = !!(system?.length || messages?.length || tools?.length ||
      requestBody.model || requestBody.max_tokens != null || requestBody.stream !== undefined ||
      requestBody.thinking || requestBody.output_config);
  ...
  return (
    <CollapsibleCard
      ...
      headerExtraContent={
        <div style={{display: 'flex', alignItems: 'center', gap: 8}}>
          <Text size="small" style={{color: 'var(--semi-color-text-2)'}}>JSON 视图</Text>
    ...
    >
      {isJsonViewMode
        ? jsonView
        : (formattedView ?? <Text type="secondary">(无请求内容)</Text>)}
    </CollapsibleCard>
  );
}
