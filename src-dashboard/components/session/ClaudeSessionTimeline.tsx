...
export default function ClaudeSessionTimeline({
  ...
  // 按 log_id 分组事件，用于 token 汇总展示
  const groups = useMemo(() => {
    const g: { logId: string; events: ConversationEvent[] }[] = [];
    let current: { logId: string; events: ConversationEvent[] } | null = null;
    for (const event of events) {
      if (!current || current.logId !== event.log_id) {
        current = {logId: event.log_id, events: []};
        g.push(current);
      }
