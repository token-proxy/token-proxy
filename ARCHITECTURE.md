...
│   └── shared/             # 跨聚合共享 (Status, ApiKey, AccessPointType + 协议方法, EncryptionService, InboundRequest, UpstreamRequest, protocols/)
├── application/            # 应用层 (用例编排, 依赖注入, 按聚合组织)
│   ├── access_point/       # AccessPoint 聚合用例 (含账户池 + 路由网格 DTO)
│   ├── auth/               # 跨聚合认证用例
│   ├── dashboard/          # Dashboard 只读视图用例 (KPI / sparkline / top users / top accounts)
│   ├── log/                # Log 聚合用例
...
│   ├── dashboard_query.rs  # Dashboard 读模型 (DashboardWindow / KpiAggregate / SparklineBucket / TopUserRow / TopAccountRow, LEFT JOIN 容忍删除)
│   ├── repository_audit_log.rs # AuditLogRepository trait
│   ├── repository_log.rs   # LogRepository trait (含 aggregate_kpi / aggregate_sparkline / top_users / top_accounts 聚合方法)
...
│   ├── api_type.rs         # AccessPointType 枚举 (Anthropic) + 5 个协议适配方法 (parse_inbound/extract_session_id/inject_api_key/replace_model_in_body/is_sse_response)
│   ├── encryption.rs       # EncryptionService trait (encrypt/decrypt)
│   ├── inbound_request.rs  # InboundRequest struct (入站请求纯数据, 无方法)
...
│   ├── mod.rs
│   ├── dashboard_service.rs # 编排服务 (加载窗口聚合 + compute_trend 纯函数处理 5 种 TrendBadge 边界)
│   ├── time_window.rs      # 时间窗口解析纯逻辑 (today/last7/last30/custom)
│   └── dto/                # 12 个 DTO: TimeRangeQuery / KpiResponse / KpiTrendItem / TrendBadge / CacheHitRate / SparklineSeries / TopUserItem / TopAccountItem 等
├── log/                    # Log 聚合用例
│   ├── mod.rs
...
│   ├── mod.rs
│   ├── proxy_pipeline.rs       # 核心代理转发管道 (60 行 execute 调度骨架 + try_one_account 子方法; 0 步关闭短路 + 协议解析 + 排序粘滞 + 候选迭代)
...
- dashboard/ 虽属跨聚合视图，但仅依赖 `LogRepository`——事实表为 `log_metadata` / `log_token_usage`，users/accounts/providers 仅作展示数据 LEFT JOIN 进同一聚合 SQL，不构成跨聚合读模型
- `ProxyCallRecord` 置于 `application/proxy/` 而非基础设施层——它直接接受领域聚合（`InboundRequest` / `UpstreamRequest` / `AccessPointEx`），层级归属应与其依赖一致
...
构造时 `start` 从 `InboundRequest`、`UpstreamRequest`、`AccessPointEx` 提取请求侧字段（model_original / model_mapped / api_type / session_id / user_id / provider_id / account_id 等），上游响应头到达后通过 `attach_response` 登记状态码和响应头，SSE 流通过 `append_body` 逐段累积、非流式通过 `set_body` 一次性写入。`finish` 计算 `duration_ms` 并构造 `ProxyLogInput`（`application/log/dto/proxy_log_input.rs`，仅作 LogService 一次性入参契约，不再是双重职责 DTO）后通过 `TrackedSpawner` 异步交给 `LogService::record_proxy_log` 落库。
...
│   ├── dashboard_routes.rs # GET /api/dashboard/kpi, /api/dashboard/top-users, /api/dashboard/top-accounts (JWT 保护)
...
│   │   ├── dashboard/              # 仪表盘组件 (8 个, 基于 Recharts)
...
│   │   │   ├── KpiCard.tsx               # 含 sparkline 的 KPI 卡
│   │   │   ├── CacheHitCard.tsx          # 纯比率 KPI 卡
│   │   │   ├── TimeRangeSelector.tsx     # RadioGroup + Popover DatePicker
│   │   │   ├── TopUsersRanking.tsx       # Top 10 成员排行卡片
│   │   │   └── TopAccountsRanking.tsx    # Top 10 账号排行卡片
│   │   └── user/                   # 用户管理组件 (1 个)
...
│   │   ├── DashboardPage.tsx       # 仪表盘概览 (CSS Grid 布局, 3 个 useFetch 并行加载 KPI/Top Users/Top Accounts, 配套 DashboardPage.css 响应式断点 1280/768)
...
│   │   ├── dashboard.ts            # 仪表盘统计类型 (镜像后端新 DTO: TimeRangeQuery / KpiResponse / KpiTrendItem / TrendBadge / SparklineSeries / TopUserItem / TopAccountItem)
...
> 类型系统强制完整性：`RetryDecision::Continue(AppError)` 强制重试时必须携带错误原因，无法 `continue` 而不带错误；过去 `last_error = Some(...); continue;` 配对易遗漏的隐患被永久消除。

### 协议适配层（domain/shared/protocols/）

LLM API 协议的差异点（请求头格式、session 标识 header 名、API key 注入方式、body 字段约定）由 `AccessPointType` 枚举挂载的 5 个协议方法（`parse_inbound` / `extract_session_id` / `inject_api_key` / `replace_model_in_body` / `is_sse_response`）封装。每个协议方法内部 `match self` 后调用对应协议模块（`protocols/<name>.rs`）中的 `pub(in crate::domain::shared) fn`。

**为什么挂在 AccessPointType 而非新建 ApiProtocol 枚举**：`AccessPointType` 本来就是协议类型的抽象（数据库列约束、`Provider::base_url_for` 已用它分发），让现有概念长出本该属于它的行为，避免并行枚举的概念膨胀。

**新增协议的工作量**：（以未来 OpenAI 为例）

1. `AccessPointType` 加 `OpenAI` 变体 + 数据库列约束 + 前端 Select
2. 新建 `protocols/openai.rs` 实现 5 个 `pub(in crate::domain::shared) fn`
...
| 日期             | 变更说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-06-23       | 代理转发链路深度重构（3 commit: 64a9147 / 3390e85 / a1ef169）: 协议适配点从单变体 enum `RequestSnapshot::Anthropic` 上移到 `AccessPointType` 枚举上挂 5 个协议方法（`parse_inbound` / `extract_session_id` / `inject_api_key` / `replace_model_in_body` / `is_sse_response`），具体实现迁到 `domain/shared/protocols/anthropic.rs`（加 OpenAI 时编译器自动指出所有 match 分支位置）。数据结构纯净化: 新增 `InboundRequest` / `UpstreamRequest` 无方法 struct, 删除 `RequestSnapshot` / `ProcessedRequest`, 变换逻辑收拢到 `AccessPointEx::build_upstream_request`。日志记录器上移到应用层: `ProxyLogger`（infrastructure/）→ `ProxyCallRecord`（application/proxy/），API 形态对应业务时序 `start → attach_response → append_body/set_body → finish`，Drop 兜底 SSE 中断；`ProxyLogData` DTO 重塑为 `ProxyLogInput`（仅 LogService 一次性入参契约）。新增 `domain/proxy/` 聚合: `UpstreamOutcome` enum 显式建模四种响应分类（Success/ClientError/Fault/ServerError）+ `classify` 统一调用 FaultService, `RetryDecision` enum (`Return(Response)` / `Continue(AppError)`) 类型强制重试时必须携带错误原因。新增应用编排子组件: `AccountSelector`（候选账号异步迭代器）、`UpstreamDispatcher`（forward + 120s 超时）、`response_builder`（流式/非流式响应构造）、`TrackedSpawner`（归一两处 fire-and-forget spawn，修复 spawn_save_affinity 缺 try_current 守卫的 panic 隐患）。`ProxyPipeline::execute` 从 320 行嵌套 if 树重写为 60 行调度骨架 + `try_one_account` 子方法（约 85 行）；session_id 从字符串 "unknown" sentinel 改为 `Option<String>`；`execute` 第 0 步检查 `shutting_down` 短路。`proxy_pipeline.rs` 总行数从 487 → 300（净减 187 行），新增 11 个单元测试（测试总数 91 → 102） |
