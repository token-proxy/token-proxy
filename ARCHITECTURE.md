...
│   ├── auth/               # 跨聚合认证用例
│   ├── dashboard/          # Dashboard 只读视图用例 (KPI / sparkline / top users / top accounts / top clients)
│   ├── log/                # Log 聚合用例
...
│   ├── mod.rs
│   ├── dashboard_service.rs # 编排服务 (加载窗口聚合 + compute_trend 纯函数处理 5 种 TrendBadge 边界 + get_top_clients)
...
│   │   ├── dashboard/              # 仪表盘组件 (9 个, 基于 Recharts)
...
│   │   │   └── TopClientsRanking.tsx     # Top Clients 排行卡片 (按 client_type 分组)
│   │   └── user/                   # 用户管理组件 (1 个)
...
│   │   ├── DashboardPage.tsx       # 仪表盘概览 (CSS Grid 布局, 4 个 useFetch 并行加载 KPI/Top Users/Top Accounts/Top Clients, 配套 DashboardPage.css 响应式断点 1280/768)
...
│   │   ├── dashboard.ts            # 仪表盘统计类型 (镜像后端新 DTO: TimeRangeQuery / KpiResponse / KpiTrendItem / TrendBadge / SparklineSeries / TopUserItem / TopAccountItem / TopClientItem)
...
**ClientType 正交概念**：`ClientType` 枚举（`ClaudeCode` / `Codex` / `Other` / `Unknown`）与 `AccessPointType` 正交——前者描述"哪个客户端在调用"，后者描述"上游走什么协议"。ClientType 由 `ProxyPipeline` 在协议解析阶段从 User-Agent 和请求路径中识别，随 `InboundRequest` 流入日志管线，最终记录到 `log_metadata.client_type` 和 `log_token_usage.client_type` 列，供 Dashboard Top Clients 聚合分析。
